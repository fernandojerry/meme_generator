...
const jokeContainer = document.querySelector('.showJoke');
const quoteContainer = document.querySelector('.showQuote');
const riddleContainer = document.querySelector('.showRiddle');

const p = document.createElement('p');
const pic = document.createElement('img');

// function that clear the screen when ever a new data is obtained
...
const showJoke = () => {
  ...
  const randomJoke = getData('jokes');
...
const showMeme = () => {
  ...
  const randomMeme = getData('memes');
  
  pic.setAttribute('src', randomMeme)
  clearScreen();
  memeContainer.appendChild(pic);
}
...
const showQuote = () => {
  ...
  const randomQuote = getData('quotes');
...
const showRiddle = () => {
  ...
  const randomRiddle = getData('riddles');
  const pAnswer = document.createElement('p');
  
  p.textContent = randomRiddle.question;
  pAnswer.textContent = "- " + randomRiddle.answer;
  pAnswer.setAttribute('id', 'revealAnswer');

  pAnswer.hidden = true;
  
  riddleContainer.appendChild(p);
  riddleContainer.appendChild(pAnswer);
}
const revealReddleAnswer = () =>{
  const riddle =riddleContainer.querySelector("p");
  
  const riddleAnswer = document.querySelector("#revealAnswer")

  if (riddle && riddleAnswer.hidden){
    riddleAnswer.hidden = false;
  }else if (riddle && riddleAnswer) {
    alert("The riddle answer is alraedy exposed");
  }else {
...
// gets a random value from data passed
